...
GIS::_normalise(const Context *context, PDF &p_classes){
  ...
  //PDF pre_p_classes(p_classes);

  //scale back
  //TODO subtract average instead of min
  //if number is smaller than -36, 0 the number
  //TODO subtract max, DOESN'T WORK
  for(ulong klass = 2; klass != NKLASSES; ++klass)
    if(p_classes[klass] > max)
      max = p_classes[klass];
  if(max < 0)
    max = 0;
  ...
  //double before_val;
  for(ulong klass = 2; klass != NKLASSES; ++klass){
    //before_val = p_classes[klass];
    p_classes[klass] = exp(p_classes[klass] - max);
    //p_classes[klass] = exp(p_classes[klass]);
    //if(isnan(p_classes[klass]))
      //nan = true;
    Z += p_classes[klass];
    //if(isnan(Z))
      //nan = true;
