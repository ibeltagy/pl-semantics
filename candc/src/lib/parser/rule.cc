...
Rules::forward_app(const SuperCat *sc1, const SuperCat *sc2, const bool eisner_nf, SuperCats &res){
  ...
  SuperCat *ressc = SuperCat::Rule(pool, rescat, SuperCat::FWD_APP, 0, sc1, sc2, unify, k);
  if(ressc)
    res.push_back(ressc);
  return true;
} 
...
bool Rules::backward_app(const SuperCat *sc1, const SuperCat *sc2, const bool eisner_nf, SuperCats &res){
  ...
  SuperCat *ressc = SuperCat::Rule(pool, rescat, SuperCat::BWD_APP, 0, sc1, sc2, unify, k);
  if(ressc)
    res.push_back(ressc);
  return true;
}
...
Rules::forward_comp_recursive(const SuperCat *sc1, const SuperCat *sc2, const bool eisner_nf, 
  ...
  if(cat2->res->res->uhash == cat1->arg->uhash){
    ...
    if(rescat){
      SuperCat *ressc = SuperCat::Rule(pool, rescat, SuperCat::FWD_COMP | SuperCat::RECURSIVE, 2, sc1, sc2, unify, k);
      if(ressc)
        res.push_back(ressc);
      return true;
    }else
    ...
    const Cat *rescat = Cat::Insert12(pool, cat1->res, at, cat2, sc1->cat->var, sc1, sc2, unify);
    if(rescat){
      SuperCat *ressc = SuperCat::Rule(pool, rescat, SuperCat::FWD_COMP | SuperCat::RECURSIVE, 3, sc1, sc2, unify, k);
      if(ressc)
        res.push_back(ressc);
      return true;
    }else
...
Rules::forward_comp(const SuperCat *sc1, const SuperCat *sc2, const bool eisner_nf, SuperCats &res){
  ...
  if(rescat){
    SuperCat *ressc = SuperCat::Rule(pool, rescat, SuperCat::FWD_COMP, 0, sc1, sc2, unify, k);
    if(ressc)
      res.push_back(ressc);
    return true;
  }else
...
Rules::backward_comp(const SuperCat *sc1, const SuperCat *sc2, const bool eisner_nf, SuperCats &res){
  ...
  if(rescat){
    SuperCat *ressc = SuperCat::Rule(pool, rescat, SuperCat::BWD_COMP, 0, sc1, sc2, unify, k);
    if(ressc)
      res.push_back(ressc);
    return true;
  }else
...
Rules::backward_cross_recursive(const SuperCat *sc1, const SuperCat *sc2, const bool eisner_nf, SuperCats &res){
  ...
  if(cat1->res->res->uhash == cat2->arg->uhash){
    ...
    if(rescat){
      SuperCat *ressc = SuperCat::Rule(pool, rescat, SuperCat::BWD_CROSS | SuperCat::RECURSIVE, 2, sc1, sc2, unify, k);
      if(ressc)
        res.push_back(ressc);
      return true;
    }else
    ...
    const Cat *rescat = Cat::Insert21(pool, cat2->res, at, cat1, sc1->cat->var, sc1, sc2, unify);
    if(rescat){
      SuperCat *ressc = SuperCat::Rule(pool, rescat, SuperCat::BWD_CROSS | SuperCat::RECURSIVE, 3, sc1, sc2, unify, k);
      if(ressc)
        res.push_back(ressc);
      return true;
    }else
...
Rules::backward_cross(const SuperCat *sc1, const SuperCat *sc2, const bool eisner_nf, SuperCats &res){
  ...
  if(rescat){
    SuperCat *ressc = SuperCat::Rule(pool, rescat, SuperCat::BWD_CROSS, 0, sc1, sc2, unify, k);
    if(ressc)
      res.push_back(ressc);
    return true;
  }else
...
Rules::backward_comp_recursive(const SuperCat *sc1, const SuperCat *sc2,
  ...
  SuperCat *ressc = SuperCat::Rule(pool, sc1->cat, SuperCat::BWD_COMP | SuperCat::RECURSIVE, 2, sc1, sc2, unify, k);
  if(ressc)
    res.push_back(ressc);
...
Rules::conj(const SuperCat *sc1, const SuperCat *sc2, SuperCats &res){
  ...
  res.push_back(SuperCat::Conj(pool, cat, SuperCat::CONJ, sc1, sc2, k));
  return true;
}
...
Rules::left_comma_typechange(const SuperCat *sc1, const SuperCat *sc2, SuperCats &res){
  ...
  if(sc2->cat->is_NP()){
    res.push_back(SuperCat::Special(pool, SbNPbSbNP, SuperCat::LEFT_TC, sc1, sc2, sc2, 56, k));
    return true;
  }
...
Rules::funny_conj(const SuperCat *sc1, const SuperCat *sc2, SuperCats &res){
  ...
  res.push_back(SuperCat::Punct(pool, sc2->cat, SuperCat::FUNNY_CONJ, sc1, sc2, sc2, k));
  return true;
}
...
Rules::gen_left_punct(const SuperCat *sc1, const SuperCat *sc2, const Cat *tb, SuperCats &res){
  if(sc1->cat->is_punct()){
    ...
    if(sc2->cat->uhash == tb->uhash){
      res.push_back(SuperCat::Punct(pool, sc2->cat, SuperCat::LEFT_PUNCT | inherited, sc1, sc2, sc2, k));
      return true;
    }else if(tb->is_bwd() &&
             sc2->cat->uhash == tb->res->uhash &&
             sc2->cat->uhash == tb->arg->uhash){
      Cat *cat = Cat::Complex(pool, sc2->cat, BWD, sc2->cat, Vars::NONE);
      res.push_back(SuperCat::Conj(pool, cat, SuperCat::LEFT_PUNCT | SuperCat::CONJ, sc1, sc2, k));
      return true;
...
Rules::right_comma_typechange(const SuperCat *sc1, const SuperCat *sc2, SuperCats &res){
  ...
  const Cat *cat1 = sc1->cat;
  if(cat1->is_NP()){
    res.push_back(SuperCat::TypeChange(pool, SfS, SuperCat::RIGHT_TC, sc1, sc2, false, 55, k));
    return true;
  }

  if(cat1->is_SdclfSdcl()){
    res.push_back(SuperCat::TypeChange(pool, SfS, SuperCat::RIGHT_TC, sc1, sc2, true, 50, k));
    res.push_back(SuperCat::TypeChange(pool, SbNPfSbNP, SuperCat::RIGHT_TC, sc1, sc2, true, 51, k));
    res.push_back(SuperCat::TypeChange(pool, SbNPbSbNP, SuperCat::RIGHT_TC, sc1, sc2, true, 52, k));
    res.push_back(SuperCat::TypeChange(pool, SbS, SuperCat::RIGHT_TC, sc1, sc2, true, 53, k));
    return true;
  }

  if(cat1->is_SdclbSdcl() && !sc1->conj()){
    res.push_back(SuperCat::TypeChange(pool, SfS, SuperCat::RIGHT_TC, sc1, sc2, true, 54, k));
    return true;
  }
...
Rules::gen_right_punct(const SuperCat *sc1, const SuperCat *sc2, const Cat *tb, SuperCats &res){
  if(sc2->cat->is_punct()){
    ...
    if(cat1->uhash == tb->uhash){
      res.push_back(SuperCat::Punct(pool, cat1, SuperCat::RIGHT_PUNCT | inherited, sc1, sc2, sc1, k));
      return true;
    }else if(tb->is_SfS()){
      if(cat1->is_NP()){
        res.push_back(SuperCat::Special(pool, SfS, SuperCat::RIGHT_TC | inherited, sc1, sc2, sc1, 13, k));
        return true;
      }

      if(cat1->is_SdclbSdcl()){
        res.push_back(SuperCat::Special(pool, SfS, SuperCat::RIGHT_TC | inherited, sc1, sc2, sc1, 15, k));
        return true;
      }
      ...
      if(cat1->is_SdclfSdcl()){
        res.push_back(SuperCat::Special(pool, SbS, SuperCat::RIGHT_TC | inherited, sc1, sc2, sc1, 16, k));
        return true;
      }
      ...
      if(cat1->is_SdclfSdcl()){
        res.push_back(SuperCat::Special(pool, SbNPbSbNP, SuperCat::RIGHT_TC | inherited, sc1, sc2, sc1, 19, k));
        return true;
      }
      ...
      if(cat1->is_SfS() && cat1->res->has_dcl() && cat1->arg->has_dcl()){
        res.push_back(SuperCat::Special(pool, SbNPfSbNP, SuperCat::RIGHT_TC | inherited, sc1, sc2, sc1, 20, k));
        return true;
      }
...
Rules::gen_misc(const SuperCat *sc1, const SuperCat *sc2, const Cat *tb, SuperCats &res){
  if(sc1->cat->is_N() && sc2->cat->is_N() && tb->is_N()){
    res.push_back(SuperCat::Punct(pool, sc2->cat, SuperCat::GEN_MISC, sc1, sc2, sc2, k));
    return true;
  }

  if(sc1->cat->uhash == tb->uhash && sc2->cat->is_SbS()){
    res.push_back(SuperCat::Punct(pool, sc1->cat, SuperCat::GEN_MISC, sc1, sc2, sc1, k));
    return true;
  }
...
Rules::appositions(const SuperCat *sc1, const SuperCat *sc2, SuperCats &res){
  ...
  if(sc1->cat->is_NP()){
    if(sc2->cat->is_NP()){
      res.push_back(SuperCat::Apposition(pool, SuperCat::APPO, sc1, sc2, k));
      return true;
    }
    return false;
  }
