...
void
BoxerPrinter::recurse(const KBest &kb, Feature parent, Sentence &sent, int depth){
  if(FORMAT & FMT_WS){
  ...
  if(sc->left){
    ...
    recurse(sc->left->kbest[kb.left], sc->feature.override(parent), sent, depth + 1);
    ...
    if(sc->right){
      ...
      if(FORMAT & FMT_WS)
        out.stream << '\n';
      assert(sc->right->nk);
      recurse(sc->right->kbest[kb.right], sc->feature.override(parent), sent, depth + 1);
      out.stream << ")";
    }else
      out.stream << ")";
  }else{
