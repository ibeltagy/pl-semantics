...
std::ostream &
Cat::out_novar_noX(std::ostream &stream, const bool brack, const bool conj) const {
  if(atom){
    stream << atom;
    ...
    if(brack)
      stream << '(';
    if(conj){
      //should be backward app and equal res and arg in a conj construction
      assert(rule_eq(res, arg) && is_bwd());
      res->out_novar_noX(stream, false, false);
      stream << "[conj]";  //stream out conj marker 
    }
    else{
