...
double
DerivsDecoder::calc_score(const SuperCat *sc){ 
  return sc->score;
}
...
ushort
DerivsDecoder::score_0(KBest ret[], const SuperCat *sc) {
  double score = calc_score(sc);
  uint x = 0;
...
ushort
DerivsDecoder::score_1(KBest ret[], const SuperCat *sc) {
  double score = calc_score(sc);
  uint x = 0;
...
double
DerivsDecoder::score_2(const SuperCat *sc){
  double score = calc_score(sc);
...
void
DerivsDecoder::get_candidates_3(const SuperCat *sc){
...
void
DerivsDecoder::lazy_next_3(std::set<KBest, std::greater<KBest> > &candidates, const KBest &kb){
...
void
DerivsDecoder::get_candidates_4(const SuperCat *sc){
...
void
DerivsDecoder::lazy_next_4(KBest ret[], ushort &nk, ushort &ncand, const KBest &kb){
  const SuperCat *l, *r, *sc = kb.sc;
  double score = calc_score(sc);
  uint index;
