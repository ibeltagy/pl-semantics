...
Chart::Chart(Categories &cats, bool EXTRA_RULES, ulong MAXWORDS, const ushort k, const ushort k_alg)
  : EXTRA_RULES(EXTRA_RULES),
    MAXWORDS(MAXWORDS), MAXCELLS(MAXWORDS*MAXWORDS/2 + MAXWORDS/2 + 1),
    pool(new Pool(1 << 22)),
    equiv("equiv"), cells(new Cell[MAXCELLS]), cats(cats),
    nwords(0), ncells(0), k(k), k_alg(k_alg),
...
Chart::build_tree(const TBSentence &sentence, ulong &node, ulong pos){
  ...
  if(current.is_internal()){
    ...
    add(pos, span, SuperCat::Lexical(pool, 0, cat, 0, k));
    return span;
  }else if(current.is_leaf()){
    const Cat *cat = cats.parse(current.cat.c_str());
    add(pos, 1, SuperCat::Lexical(pool, pos, cat, 0, k));
    return 1;
  }else
  ...
  if(TBcat->is_NP()){
    ...
    for(TRCats::const_iterator j = trcats->begin(); j != trcats->end(); ++j)
      ...
      if((*j).cat->uhash == TBres->uhash){
        cell.add(SuperCat::TypeRaise(pool, *j, SuperCat::TR, sc, 11, k));
        return true;
      }
    return false;
  }
...
Chart::tr(Position pos, Position span){
  ...
  for(ulong i = current.nold; i != current.size(); ++i){
    ...
    for(TRCats::const_iterator j = trcats->begin(); j != trcats->end(); ++j){
      SuperCat *new_sc = SuperCat::TypeRaise(pool, *j, SuperCat::TR, sc, 11, k);
...
Chart::_add_lex(Position pos, Position span, const Cat *cat, const SuperCat *sc, bool replace, RuleID ruleid){
  SuperCat *new_sc = SuperCat::LexRule(pool, cat, SuperCat::LEX, sc, replace, ruleid, k);
  if(equiv.add(pos, span, new_sc))
    tmp.add(new_sc);
}
...
Chart::_add_genlex(Cell &cell, const Cat *cat, const SuperCat *sc, bool replace, RuleID ruleid){
  cell.add(SuperCat::LexRule(pool, cat, SuperCat::LEX, sc, replace, ruleid, k));
  return true;
}
...
void
Chart::_count_deps(const SuperCat *sc, Sentence &sent){
  for(; sc; sc = sc->next){
    if(!sc->deps_counted()){
      sc->count_deps(cats.markedup, cats.relations, sent);
      if(sc->left){
        _count_deps(sc->left, sent);
        if(sc->right)
          _count_deps(sc->right, sent);
      }
    }
  }
}
...
void
Chart::count_deps(Sentence &sent){
  for(Cell::iterator it = root().begin(); it != root().end(); ++it)
    _count_deps(*it, sent);
}
