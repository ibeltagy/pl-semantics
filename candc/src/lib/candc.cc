...
CandC::Config::Config(const OpPath *base,
  ...
  : Directory(name, desc, base),
    ...
    printer(*this, "printer", "parser printing style [deps, grs, prolog, boxer, ccgbank, xml, debug, js, gold_deps]", "grs"),
    decoder(*this, "decoder", "parser decoder [deps, derivs, derivs_unique, gold_deps, random]", "derivs"),
...
CandC::CandC(Config &cfg, const std::string &PREFACE)
  : cfg(cfg),
    ...
    integration(cfg.integration, cfg.pos, cfg.super, cfg.parser, sent),
    decoder(cfg.decoder(), cfg.parser.kbest(), cfg.parser.kbest_algorithm()),
...
CandC::oracle(IO::Input &in, IO::Input &constraints, IO::Output &out, IO::Log &log,
	...
	while(read_buffer(in.stream)){
		...
		integration.parse(sent, decoder, printer, ret, false); // don't use the supertagger
	}
