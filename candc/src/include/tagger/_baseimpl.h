...
Tagger::Impl::_calc_viterbi_word(State &state, ulong i, const ulong DICT_CUTOFF) const {
  ...
  if(state.gold_super.size() > 0 && state.gold_super[i] != NONE && std::find(permit->begin(), permit->end(), state.gold_super[i]) == permit->end()){
    tags = *permit;
    tags.push_back(state.gold_super[i]);
    permit = &tags;
  }
...
Tagger::Impl::_unpack_tags(State &state, Raws &seq, bool RECORD) const {
  ...
  seq.resize(LEN);
  //const bool UNKNOWN_GOLD_POS = (state.gold_pos.size() > 0); // for unknown assign gold pos experiment
  const bool UNKNOWN_GOLD_POS = false;
  for(int i = LEN - 1; i >= 0; --i, path = path->prev){
    if(state.words[i].freq() == 0 && UNKNOWN_GOLD_POS)
      seq[i] = klasses[state.gold_pos[i]];
    else
