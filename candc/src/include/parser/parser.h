  ...
  namespace CCG {
    ...
    public:
      ...
      public:
        ...
        Restricted<ushort> kbest;
        Restricted<ushort> kbest_algorithm;
    ...
    public:
      ...
      ushort best(KBest ret[], Decoder &decoder);
      void calc_stats(Statistics &stats);

      void dump_chart(std::ostream &out) const;

      void load_gold_deps(void);

      Sentence &sentence(void);
      Chart &chart(void);
      Rules &rules(void);

      static void kbest_check(const ushort &alg);
      static void alg_check(const ushort &alg);
    private:
      class _Impl;
      _Impl *_impl;
    };
