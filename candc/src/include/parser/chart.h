...
namespace NLP {
  namespace CCG {
    ...
    public:
      ...
      Chart(Categories &cats, bool EXTRA_RULES, ulong MAXWORDS, const ushort k = 1, const ushort k_alg = 0);
      ...
      void create(Pool *pool, Position pos, const Cat *cat){
        if(cell(pos, 1).has_leaf(cat))
          return;
        add(pos, 1, SuperCat::Lexical(pool, pos+1, cat, 0, k));
      }
    ...
    private:
      ...
      bool _add_genlex(Cell &cell, const Cat *cat, const SuperCat *sc,
           bool replace, RuleID ruleid);
      void _count_deps(const SuperCat *sc, Sentence &sent);
    public:
