...
Implicativity and factivity are concerned with analyzing
...
\entpairex{ex:imp-fact-nested}{Ed did not forget to arrange that Dave 
fail. [TODO: THis is a dumb sentence]\footnote{Example
\eqref{ex:imp-fact-nested} is derived from examples by \citet{maccartney:iwcs2009}}}{Dave failed.}
...
\begin{figure}[t]
		...
		\drs{~x0~}{
		  ...
		  ~\drs{~p2~}{
		    ~\negdrs{~e1~}{
		      ...
		      ~theme(e1, p2)~ \\
		      ~agent(e1, x0)~} \\
		    \unboxedifdrs{\POS{p2}}{   \drs{~e3~}{~\pred{leave}{1006}(e3)~ \\ ~agent(e3, x0)~}} \\
  ...
  \subfloat[Subexpression-triggering inference rules for implicative ``manage
    ...
    \begin{minipage}[c][1.6\width]{0.4\textwidth}
	  \centering
	    \shortstack{
	      \unboxedifdrs{\drs{~p~}{~   \drs{~e~}{~\pred{manage}{1004}(e)~ \\ ~theme(e, p)~}~}}{\POS{p}} \\
	      \\\\\\\\\\\\
	      \unboxedifdrs{\drs{~p~}{~\negdrs{~e~}{~\pred{manage}{1004}(e)~ \\ ~theme(e, p)~}~}}{\NEG{p}}
	    }
    \end{minipage}
  }
      ...
      \centering
		\drs{~x0~}{
		  ...
		  ~\drs{~e1 p2~}{
		    ...
		    \negdrs{ }{~agent(e1, x0)~} \\
		    \unboxedifdrs{\POS{p2}}{\drs{~e3~}{~\pred{              leave}{1006}(e3)~ \\ ~agent(e3, x0)~}} \\
		    \unboxedifdrs{\NEG{p2}}{\drs{~e3~}{~\pred{\textbf{not\_}leave}{1006}(e3)~ \\ \negdrs{ }{~agent(e3, x0)~}}} }
		}
    \end{minipage}
  }
  ~~~~~~~~~
    ...
    \begin{minipage}[c][1.8\width]{0.4\textwidth}
	  \centering
	    \shortstack{
	      \unboxedifdrs{\drs{~p~}{~\drs{~e~}{~              \pred{manage}{1004}(e)~ \\ ~theme(e, p)~}~}}{\POS{p}} \\
	      \\\\\\\\\\\\
	      \unboxedifdrs{\drs{~p~}{~\drs{~e~}{~\textbf{not\_}\pred{manage}{1004}(e)~ \\ ~theme(e, p)~}~}}{\NEG{p}}
	    }
    \end{minipage}
  }
...
\subsection*{Interaction of various phenomena}

[Examples with implicatives and negations and ambiguous words]
