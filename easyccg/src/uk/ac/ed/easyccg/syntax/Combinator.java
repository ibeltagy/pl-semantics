...
public abstract class Combinator
{
  ...
  public abstract Pair<Category, Category> modifyForBoxer(Category node, Category left, Category right);
  ...
  private static Category correctWildCardFeatures(Category toCorrect, Category match1, Category match2) {
	  Category c = toCorrect.doSubstitution(match1.getSubstitution(match2));
/*	  if (isPrinting)
	  {
		  Category matchedCat = match1.match(match2);
		 // match1.setFeatureStoX();
		  //match2.setFeatureStoX();
		  System.out.println(">>>>>>>>>>>>>>" + toCorrect + ", " + c + ", " + match1 + ", " + match2 + ", " +matchedCat );
	  }
*/
    return c;
  }
  ...
  private static class Conjunction extends Combinator {
    ...
    @Override
    public Pair<Category, Category> modifyForBoxer(Category node, Category left, Category right)
    {
    	//do nothing
    	return new Util().new Pair<Category, Category>(left, right);
    }
  ...
  private static class ForwardApplication extends Combinator {
    ...
    public Pair<Category, Category> modifyForBoxer(Category node, Category left, Category right)
    {
    	...
    	/*if (left.isModifier()) // Even if it is a modifier, you still need to match node with left.left
        {
        	matched = node.match(right);
        	right  = matched;
        }
        else
        {*/
        	matched = node.match(left.getLeft());
        	left = Category.make(matched, left.getSlash(), left.getRight());
        /*
        }*/

        //---
    	matched = left.getRight().match(right);
    	left = Category.make(left.getLeft(), left.getSlash(), matched);
    	right = matched;
  ...
  private static class BackwardApplication extends Combinator {
    ...
    public Category apply(Category left, Category right)
    {
      ...
      result = correctWildCardFeatures(result, right.getRight(), left);
      return result;
    }
    ...
    public Pair<Category, Category> modifyForBoxer(Category node, Category left, Category right)
    {
    	...
    	//----
    	/*
        if (right.isModifier()) //even if right is a modifier, you still need to match node with right.left
        {
        	matched = node.match(left);
        	left  = matched;
        }
        else
        {*/
        	matched = node.match(right.getLeft());
        	right = Category.make(matched, right.getSlash(), right.getRight());
        /*}*/

        matched = right.getRight().match(left);
    	left = matched;
    	right = Category.make(right.getLeft(), right.getSlash(), matched);
  ...
  private static class ForwardComposition extends Combinator {
    ...
    public Category apply(Category left, Category right)
    {
      ...
      result = correctWildCardFeatures(result, right.getLeft(), left.getRight());
      return result;
    }
    ...
    public Pair<Category, Category> modifyForBoxer(Category node, Category left, Category right)
    {
    	Category matched = right.getLeft().match(left.getRight());
    	left = Category.make(left.getLeft(), left.getSlash(), matched);
    	right = Category.make(matched, right.getSlash(), right.getRight());
    	

    	matched = node.match( Category.make(left.getLeft(), resultSlash, right.getRight()));
    	left= Category.make(matched.getLeft(), left.getSlash(), left.getRight());
    	right = Category.make(right.getLeft(), right.getSlash(), matched.getRight());
    	return new Util().new Pair<Category, Category>(left, right);
    }
  ...
  private static class GeneralizedForwardComposition extends Combinator {
    ...
    public Pair<Category, Category> modifyForBoxer(Category node, Category left, Category right)
    {
    	Category matched = right.getLeft().getLeft().match(left.getRight());
    	left = Category.make(left.getLeft(), left.getSlash(), matched);
    	right = Category.make(Category.make(matched, right.getLeft().getSlash(), right.getLeft().getRight()) , right.getSlash(), right.getRight());
  ...
  private static class GeneralizedBackwardComposition extends Combinator {
    ...
    public Pair<Category, Category> modifyForBoxer(Category node, Category left, Category right)
    {
    	Category matched = left.getLeft().getLeft().match(right.getRight());
    	left = Category.make(Category.make(matched, left.getLeft().getSlash(), left.getLeft().getRight()) , left.getSlash(), left.getRight());
    	right = Category.make(right.getLeft(), right.getSlash(), matched);
    	throw new RuntimeException();
    	//return new Util().new Pair<Category, Category>(left, right);
    }
  ...
  private static class BackwardComposition extends Combinator {
    ...
    public Category apply(Category left, Category right)
    {
      ...
      result = correctWildCardFeatures(result, left.getLeft(), right.getRight());
      return result;
    ...
    public Pair<Category, Category> modifyForBoxer(Category node, Category left, Category right)
    {
    	Category matched = left.getLeft().match(right.getRight());
    	left = Category.make(matched, left.getSlash(), left.getRight());
    	right = Category.make(right.getLeft(), right.getSlash(), matched);
    	//---
        /*if (right.isModifier())  //even if it a modifier, you still need it
        {
        	matched = node.match(left);
        	left  = matched;
        }
        ...
        {*/
        	matched = node.match( Category.make(right.getLeft(), resultSlash, left.getRight()));
        	left= Category.make(left.getLeft(), left.getSlash(), matched.getRight());
        	right = Category.make(matched.getLeft(), right.getSlash(), right.getRight());
        /*}*/
    	//---
