...
public abstract class Category {
  ...
  static class FunctorCategory extends Category {
    ...
    @Override
    Category match(Category arg)
    {
    	assert(this.left == arg.getLeft() || this.right == arg.getRight());
    	if (this.left != arg.getLeft())
    	{
    		Category changedLeft = this.left.match(arg.getLeft());
    		if (changedLeft  == this.left)
    			return this;
    		else if (changedLeft == arg.getLeft())
    			return arg;
    		else throw new RuntimeException("Error");
    	}
    	else if (this.right!= arg.getRight())
    	{
    		Category changedRight = this.right.match(arg.getRight());
    		if (changedRight == this.right)
    			return this;
    		else if (changedRight == arg.getRight())
    			return arg;
    		else throw new RuntimeException("Error");
    	}
    	else return this;
  ...
  abstract Category match (Category arg);
  ...
  static class AtomicCategory extends Category {
    ...
    Category match(Category arg)
    {
    	assert (this.getType() == arg.getType());
    	if (this.getType().equals("S")  || this.getType().equals("NP"))
    	{
