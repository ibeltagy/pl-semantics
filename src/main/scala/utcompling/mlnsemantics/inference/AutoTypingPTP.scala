        	...
        	else
        	{
        	  if(first) //add constants only in the first iteration
        	  {
				 //repeat = true;   //TODO: this should be uncommented but after reducing constatns from SetGoal, and fix AutoTyping
        	      ...
        	      {
	        	    val allConstantsOfType = (
	        	        if (quantifiedVars.contains(args.head.name))
	        	        	allConstants(autoConst(pred.name)._1.head)
		        	    else 
		        	    	allConstants(autoConst(pred.name)._1.last)
	        	    )
        	        autoConst(pred.name)._2  ++=  allConstantsOfType.map( c => {
        	        	("CONST", args.map(arg=>{
	        	    	  if(quantifiedVars.contains(arg.name)) c else arg.name
	        	      }))
        	        })
